...
class RecipeTextParser {
    ...
    // MARK: - Ingredient Parsing (internal so RecipeURLImporter can use it)
    
    /// Parse a single ingredient string like "1 1/2 cups all-purpose flour" into an Ingredient
    static func parseIngredientLine(_ line: String) -> Ingredient? {
        let cleaned = line
    ...
    static func isQuantity(_ str: String) -> Bool {
        ...
        for (index, pattern) in patterns.enumerated() {
            if let match = lower.range(of: pattern, options: .regularExpression) {
                ...
                case 0:
                    ...
                    return high * 60
                case 1: return first * 60
                case 2: return first * 3600
                case 3: return first
                default: break
                }
            }
