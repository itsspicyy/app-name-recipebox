...
struct CookingModeView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .onTapGesture {
                // Tap anywhere to advance when voice mode is on
                if voiceEnabled && currentPhase != .ingredients && currentPhase != .done {
                    goNext()
                }
            }
        }
    ...
    private var topBar: some View {
        HStack {
            Button { timer?.invalidate(); speaker.stop(); dismiss() } label: {
                Image(systemName: "xmark.circle.fill").font(.title2).foregroundStyle(.white.opacity(0.6))
            }
            Spacer()
            VStack(spacing: 2) {
                Text(recipe.title).font(.subheadline.weight(.semibold)).foregroundStyle(.white)
                Text(currentPhase.rawValue).font(.caption).foregroundStyle(Color(hex: "2A9D8F"))
            }
            Spacer()
            HStack(spacing: 12) {
                // Voice toggle
                Button {
                    ...
                    else if currentPhase != .ingredients && currentPhase != .done {
                        speakCurrentStep()
                    }
                } label: {
                    Image(systemName: voiceEnabled ? "speaker.wave.3.fill" : "speaker.slash.fill")
                        .font(.body).foregroundStyle(voiceEnabled ? Color(hex: "2A9D8F") : .white.opacity(0.4))
                }
                
                if currentPhase != .ingredients && currentPhase != .done {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                ForEach(recipe.ingredients) { ing in
                    VStack(alignment: .leading, spacing: 4) {
                        HStack(spacing: 12) {
                            Image(systemName: "circle").font(.caption).foregroundStyle(Color(hex: "2A9D8F"))
                            Text(ing.displayText).font(.title3).foregroundStyle(.white)
                        }
                        if let sub = ing.substitution, !sub.isEmpty {
                            HStack(spacing: 6) {
                                Image(systemName: "arrow.triangle.swap").font(.caption2)
                                Text("Sub: \(sub)").font(.caption)
                            }.foregroundStyle(Color(hex: "F4A261")).padding(.leading, 28)
                        }
                    }.padding(.vertical, 4)
                }
    ...
    private var stepView: some View {
        ...
        return AnyView(
            VStack(spacing: 24) {
                HStack(spacing: 6) {
                    Image(systemName: current.phase == .preparation ? "hands.sparkles.fill" : "flame.fill")
                    Text(current.phase.rawValue.uppercased())
                }.font(.caption.weight(.bold)).tracking(2).foregroundStyle(Color(hex: "E76F51"))
                
                Text("Step \(currentStepIndex + 1)").font(.headline).foregroundStyle(.white.opacity(0.5))
                
                Text(current.step.text).font(.title2.weight(.medium)).foregroundStyle(.white)
                    .multilineTextAlignment(.center).lineSpacing(6).padding(.horizontal)
                
                if voiceEnabled {
                    Text("Tap anywhere to go to the next step")
                        .font(.caption).foregroundStyle(.white.opacity(0.3))
                }
                
                if let secs = current.step.timerSeconds, secs > 0, !timerActive {
                    Button { HapticManager.medium(); startTimer(seconds: secs) } label: {
                        ...
                        .background(Color(hex: "E76F51")).clipShape(Capsule())
                    }
                }
            }
        )
            ...
            VStack(spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ForEach(1...5, id: \.self) { star in
                        Button { HapticManager.selection(); postCookRating = star } label: {
                            Image(systemName: (postCookRating ?? 0) >= star ? "star.fill" : "star")
                                .font(.system(size: 36))
                ...
                Button {
                    store.logCookingSession(for: recipe, rating: postCookRating); dismiss()
                } label: {
                    Text(postCookRating != nil ? "Save & Close" : "Skip & Close")
    ...
    private var timerDisplay: some View {
        VStack(spacing: 8) {
            ...
            Button { timer?.invalidate(); timerActive = false; timerSeconds = 0 } label: {
                Text("Cancel").font(.caption.weight(.medium)).foregroundStyle(.white.opacity(0.6))
            }
        ...
        HStack(spacing: 20) {
            if currentPhase != .ingredients {
                Button { goBack() } label: {
                    ...
                    .font(.body.weight(.medium)).foregroundStyle(.white.opacity(0.7))
            ...
            if currentPhase != .done {
                Button { goNext() } label: {
                    HStack(spacing: 4) { Text(currentPhase == .ingredients ? "Start" : "Next"); Image(systemName: "chevron.right") }
                    .font(.body.weight(.semibold)).foregroundStyle(.white)
                    .padding(.horizontal, 32).padding(.vertical, 14)
                    .background(Color(hex: "2A9D8F")).clipShape(Capsule())
                }
    ...
    private func goNext() {
        ...
        if currentPhase == .ingredients {
            ...
            if !allSteps.isEmpty { currentStepIndex = 0; currentPhase = allSteps[0].phase }
            else { currentPhase = .done }
        } else if currentStepIndex < allSteps.count - 1 {
            HapticManager.selection()
            currentStepIndex += 1; currentPhase = allSteps[currentStepIndex].phase
        } else { HapticManager.success(); currentPhase = .done }
    ...
    private func goBack() {
        ...
        if currentPhase == .done {
            if !allSteps.isEmpty { currentStepIndex = allSteps.count - 1; currentPhase = allSteps[currentStepIndex].phase }
    ...
    private func speakCurrentStep() {
        guard currentPhase != .ingredients && currentPhase != .done else { return }
        guard currentStepIndex < allSteps.count else { return }
        let text = "Step \(currentStepIndex + 1). \(allSteps[currentStepIndex].step.text)"
        speaker.speak(text)
    }
    ...
    private func startTimer(seconds: Int) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { t in
            if timerSeconds > 0 { timerSeconds -= 1 }
                ...
                t.invalidate()
                if voiceEnabled { speaker.speak("Timer is done.") }
            }
        }
    }
