...
enum RecipeStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .saveFailed(let file, _):
            return "Failed to save \(file). Your changes may not persist after closing the app."
        case .loadFailed(let file, _):
            return "Failed to load \(file). Starting with an empty list."
        }
    }
}
...
class RecipeStore: ObservableObject {
    ...
    /// Non-fatal errors the UI can surface to the user.
    @Published var lastError: RecipeStoreError? = nil
    ...
    func recipesInCollection(_ n: String) -> [Recipe] {
        recipes.filter { $0.collections.contains(n) }
    }

    var mealSuggestions: [MealSuggestionEngine.ScoredRecipe] {
        MealSuggestionEngine.suggest(from: recipes, limit: 3)
    }

    var topSuggestion: MealSuggestionEngine.ScoredRecipe? {
        MealSuggestionEngine.topSuggestion(from: recipes)
    }

    // MARK: - CRUD

    func addRecipe(_ recipe: Recipe) {
        var r = recipe; r.dateCreated = Date(); r.dateModified = Date()
        recipes.append(r)
        saveRecipes()
    }
    ...
    func updateRecipe(_ recipe: Recipe) {
        ...
        var u = recipe; u.dateModified = Date()
        recipes[i] = u
        saveRecipes()
    }
    ...
    func rateRecipe(_ recipe: Recipe, rating: Int) {
        guard let i = recipes.firstIndex(where: { $0.id == recipe.id }) else { return }
    ...
    func logCookingSession(for recipe: Recipe, rating: Int? = nil) {
        ...
        if let r = rating { recipes[i].rating = r }
        saveRecipes()
    }

    func importRecipe(_ recipe: Recipe) {
        if recipes.contains(where: { $0.title == recipe.title }) {
            var r = recipe; r.title += " (Shared)"; addRecipe(r)
        } else {
            addRecipe(recipe)
        }
    }
    ...
    func loadVersion(_ version: RecipeVersion, into recipeID: UUID) {
        ...
        recipes[i].ingredients      = version.ingredients
        recipes[i].preparationSteps = version.preparationSteps
        recipes[i].cookingSteps     = version.cookingSteps
        recipes[i].notes            = version.notes
        recipes[i].activeVersionID  = version.id
        recipes[i].dateModified     = Date()
        saveRecipes()
    }

    // MARK: - Collections

    func addCollection(_ n: String) {
        guard !n.isEmpty, !collections.contains(n) else { return }
        collections.append(n)
        saveCollections()
    }

    func removeCollection(_ n: String) {
        collections.removeAll { $0 == n }
        for i in recipes.indices { recipes[i].collections.removeAll { $0 == n } }
        saveCollections()
        saveRecipes()
    }

    func addRecipeToCollection(_ r: Recipe, collection: String) {
        guard let i = recipes.firstIndex(where: { $0.id == r.id }) else { return }
        if !recipes[i].collections.contains(collection) {
            recipes[i].collections.append(collection)
            saveRecipes()
        }
    }

    func removeRecipeFromCollection(_ r: Recipe, collection: String) {
        guard let i = recipes.firstIndex(where: { $0.id == r.id }) else { return }
        recipes[i].collections.removeAll { $0 == collection }
        saveRecipes()
    }
    ...
    func generateGroceryList(for selected: [Recipe]) {
        ...
        for recipe in selected {
            for ing in recipe.ingredients {
                ...
                let qty = [ing.quantity, ing.unit].filter { !$0.isEmpty }.joined(separator: " ")
                if var ex = merged[key] {
                    if !qty.isEmpty { ex.quantities.append(qty) }
                    merged[key] = ex
                } else {
                    merged[key] = GroceryItem(
                        name: ing.name,
                        quantities: qty.isEmpty ? [] : [qty],
                        aisle: GroceryAisle.detect(for: ing.name),
                        inPantry: isInPantry(ing.name)
                    )
                }
            }
        }
    ...
    func toggleGroceryItem(_ item: GroceryItem) {
        guard let i = groceryList.firstIndex(where: { $0.id == item.id }) else { return }
        groceryList[i].isChecked.toggle()
    }

    func clearGroceryList() {
        groceryList = []
        grocerySourceRecipes = []
    }
    ...
    func addPantryItem(_ name: String) {
        guard !name.trimmingCharacters(in: .whitespaces).isEmpty else { return }
        let item = PantryItem(name: name.trimmingCharacters(in: .whitespaces))
        guard !pantryItems.contains(where: { $0.normalizedName == item.normalizedName }) else { return }
    ...
    func addToMealPlan(recipeID: UUID, date: Date, mealType: MealPlanEntry.MealType) {
        mealPlan.append(MealPlanEntry(recipeID: recipeID, date: date, mealType: mealType))
        saveMealPlan()
    }
    ...
    func mealPlanEntries(for date: Date) -> [MealPlanEntry] {
        mealPlan
            .filter { Calendar.current.isDate($0.date, inSameDayAs: date) }
            .sorted { $0.mealType.rawValue < $1.mealType.rawValue }
    }

    func recipeForEntry(_ entry: MealPlanEntry) -> Recipe? {
        recipes.first(where: { $0.id == entry.recipeID })
    }

    func generateGroceryListFromMealPlan(for week: [Date]) {
        let entries = mealPlan.filter { e in
            week.contains(where: { Calendar.current.isDate($0, inSameDayAs: e.date) })
        }
    ...
    private func save<T: Encodable>(_ value: T, to filename: String) {
        do {
            ...
            try data.write(to: docURL(filename), options: .atomic)
        } catch {
            DispatchQueue.main.async {
                self.lastError = .saveFailed(filename, error)
            }
            print("RecipeStore save failed [\(filename)]: \(error)")
        }
    }

    private func load<T: Decodable>(_ filename: String) -> T? {
        let url = docURL(filename)
        ...
        do {
            ...
            DispatchQueue.main.async {
                self.lastError = .loadFailed(filename, error)
            }
            print("RecipeStore load failed [\(filename)]: \(error)")
            return nil
        }
    }
